...
from backtesting.validation import ValidationAnalyzer
matplotlib.use('Agg')
import logging
from datetime import datetime, timedelta
import pytz
from strategies.registry import STRATEGY_REGISTRY, get_strategy
from backtesting.data import get_data_sync
from backtesting.reports import PerformanceAnalyzer
import json
import numpy as np
from backtesting.parameter_optimization import optimize_strategy
...
# Default ticker configuration - can be easily modified or loaded from external source
DEFAULT_TICKERS = [
...
# Remove all demo-related code and strings from the codebase
    """Get the default ticker for demo purposes."""
    available_tickers = get_available_tickers()
    return available_tickers[0]
...
def plot_contour(study):
    ...
    try:
        if not study.trials or all(t.state != optuna.trial.TrialState.COMPLETE for t in study.trials):
            logger.warning("No valid trials available for contour plot")
            st.error("No valid trials available for contour plot.")
            return None
        ...
        st.error(f"Failed to generate contour plot: {str(e)}")
        return None
...
def analyze_best_trades(results):
    ...
    try:
        ...
        if trades:
            ...
            for t in trades:
                try:
                    ...
                    trade_info = {
                        ...
                        'entry_date': str(entry_date) if entry_date is not None else '-',
                        ...
                        'return_pct': (pnl / (t.get('price_in', 1) * abs(size))) * 100 if t.get('price_in', 0) > 0 and size != 0 else 0
                    }
            ...
            if trade_list:
                ...
                return {
                    ...
                    "total_pnl": total_pnl,
                    "avg_winning_trade": sum(trade['pnl'] for trade in winning_trades) / len(winning_trades) if winning_trades else 0,
                    "avg_losing_trade": sum(trade['pnl'] for trade in losing_trades) / len(losing_trades) if losing_trades else 0,
        ...
        trades = None
        if hasattr(strategy, 'analyzers') and hasattr(strategy.analyzers, 'tradeanalyzer'):
        ...
        if isinstance(closed_trades, list) and closed_trades and isinstance(closed_trades[0], dict):
            for i, trade in enumerate(closed_trades):
                try:
                    entry_date = pd.to_datetime(trade['datein'], unit='s').tz_localize('UTC').tz_convert(IST)
                    exit_date = pd.to_datetime(trade['dateout'], unit='s').tz_localize('UTC').tz_convert(IST)
                    pnl = trade.get('pnl', 0)
                    ...
                    trade_info = {
                        ...
                        'return_pct': (pnl / (trade['pricein'] * trade.get('size', 1))) * 100 if trade['pricein'] > 0 else 0
                    }
                    trade_list.append(trade_info)
        ...
        for attr_name in dir(strategy):
            if not attr_name.startswith('_'):  # Skip private attributes
                ...
                if hasattr(attr, '__class__') and hasattr(attr.__class__, '__module__'):
                    ...
                    # Check if it's a backtrader indicator
                    if 'backtrader' in module_name and 'indicator' in module_name.lower():
...
def create_candlestick_chart_with_trades(data, results, title="Candlestick Chart with Trades"):
    ...
    try:
        ...
        price_indicators = [k for k, v in calculated_indicators.items() if v['subplot'] == 'price']
        oscillator_indicators = [k for k, v in calculated_indicators.items() if v['subplot'] == 'oscillator']
        ...
        if oscillator_indicators:
            num_rows = 4
            subplot_titles = ["Price Action & Trades", "Technical Indicators", "Volume", "Equity Curve"]
        ...
        dates = filtered_data.index.tz_localize('UTC').tz_convert(IST) if filtered_data.index.tz is None else filtered_data.index.tz_convert(IST)
            ...
            for indicator_name, indicator_data in calculated_indicators.items():
                if indicator_data['subplot'] == 'oscillator':
                    ...
                    if 'y_range' in indicator_data:
                        fig.update_yaxes(range=indicator_data['y_range'], row=oscillator_row, col=1)
        ...
        elif hasattr(strategy, 'analyzers') and hasattr(strategy.analyzers, 'time_return'):
                ...
                if isinstance(equity, dict):
                    ...
                    if equity_keys:
                        ...
                        if isinstance(first_key, str):
                            equity_dates = pd.to_datetime(equity_keys, utc=True).tz_convert(IST)
                        elif isinstance(first_key, (pd.Timestamp, datetime)):
                            if first_key.tzinfo is None:
                                equity_dates = pd.to_datetime(equity_keys).tz_localize('UTC').tz_convert(IST)
                            else:
                                equity_dates = pd.to_datetime(equity_keys).tz_convert(IST)
                        else:
                            equity_dates = pd.to_datetime(equity_keys, unit='s', utc=True).tz_convert(IST)
        ...
        fig.update_xaxes(
            rangebreaks=[
                dict(bounds=[6, 0], pattern="day of week"),  # Hide weekends (Saturday=5, Sunday=6)
                dict(bounds=["15:30", "09:15"], pattern="hour")  # Hide non-trading hours
            ]
        )
...
def create_trades_with_indicators_table(results, data, strategy):
    """Create a table of all trades with entry/exit prices, times, and indicator values."""
    try:
        trades = None
        if hasattr(strategy, 'analyzers') and hasattr(strategy.analyzers, 'tradeanalyzer'):
            trades = strategy.analyzers.tradeanalyzer.get_analysis()
        ...
        closed_trades = trades.get('closed', []) or trades.get('trades', [])
        if not (isinstance(closed_trades, list) and closed_trades):
            return pd.DataFrame(), "No valid trades found"
        ...
        for i, trade in enumerate(closed_trades):
            try:
                entry_dt = pd.to_datetime(trade['datein'], unit='s').tz_localize('UTC').tz_convert(IST)
                exit_dt = pd.to_datetime(trade['dateout'], unit='s').tz_localize('UTC').tz_convert(IST)
                # Find closest index in data for entry/exit
                entry_idx = data.index.get_indexer([entry_dt], method='nearest')[0]
                exit_idx = data.index.get_indexer([exit_dt], method='nearest')[0]
                row = {
                    'Trade #': i + 1,
                    ...
                    'P&L': trade.get('pnl', 0),
                    'Size': int(abs(trade.get('size', 1))),
                    'Direction': 'Long' if trade.get('size', 1) > 0 else 'Short',
                }
                # Add indicator values at entry/exit
                for ind_name, ind_data in calculated_indicators.items():
                    entry_val = ind_data['values'].iloc[entry_idx] if entry_idx < len(ind_data['values']) else None
                    exit_val = ind_data['values'].iloc[exit_idx] if exit_idx < len(ind_data['values']) else None
                    row[f'{ind_data["name"]} (Entry)'] = entry_val
                    row[f'{ind_data["name"]} (Exit)'] = exit_val
                trade_rows.append(row)
            except Exception as e:
                logger.warning(f"Error processing trade for indicators: {e}")
                continue
        if trade_rows:
            return pd.DataFrame(trade_rows), None
        return pd.DataFrame(), "No valid trades found"
    except Exception as e:
        logger.error(f"Error creating trades+indicators table: {e}")
        return pd.DataFrame(), f"Error creating trades+indicators table: {str(e)}"

def extract_trades(strategy_result: bt.Strategy, data: pd.DataFrame = None) -> pd.DataFrame:
    ...
    try:
        ...
        calculated_indicators = calculate_indicator_values(data, detected_indicators) if data is not None else {}
        ...
        if hasattr(strategy_result, "broker") and hasattr(strategy_result.broker, "orders"):
            orders = strategy_result.broker.orders
            completed_orders = [order for order in orders if order.status == 4]  # Status 4 = Completed
            ...
            for order in completed_orders:
                if hasattr(order, "executed") and order.executed.size != 0:
                    ...
                    if data is not None and calculated_indicators:
                        closest_idx = data_index.get_indexer([exec_dt], method='nearest')[0]
                        for ind_name, ind_data in calculated_indicators.items():
                            ind_value = ind_data['values'].iloc[closest_idx] if closest_idx < len(ind_data['values']) else None
                    ...
                    if exec_size > 0:  # Buy order (Long entry or Short exit)
                        if open_short_positions:  # Closing short position
                            ...
                            if data is not None and calculated_indicators:
                                ...
                                closest_idx = data_index.get_indexer([entry_dt], method='nearest')[0]
                                for ind_name, ind_data in calculated_indicators.items():
                                    ind_value = ind_data['values'].iloc[closest_idx] if closest_idx < len(ind_data['values']) else None
        ...
        if not trades and hasattr(strategy_result, "_trades"):
            for trade_obj in strategy_result._trades:
                if hasattr(trade_obj, "isclosed") and trade_obj.isclosed:
                    try:
                        entry_dt = bt.num2date(trade_obj.dtopen) if hasattr(trade_obj, "dtopen") else None
                        exit_dt = bt.num2date(trade_obj.dtclose) if hasattr(trade_obj, "dtclose") else None
                        ...
                        trade_info = {
                            ...
                            "exit_price": (
                                getattr(trade_obj, "price", 0) + (
                                    getattr(trade_obj, "pnl", 0) / getattr(trade_obj, "size", 1)
                                ) if getattr(trade_obj, "size", 1) != 0 else getattr(trade_obj, "price", 0)
                            ),
                            "size": abs(getattr(trade_obj, "size", 0)),
                            "pnl": getattr(trade_obj, "pnl", 0),
                            "pnl_net": getattr(trade_obj, "pnlcomm", getattr(trade_obj, "pnl", 0)),
                            "commission": getattr(trade_obj, "commission", 0),
                            "status": "Won" if getattr(trade_obj, "pnl", 0) > 0 else "Lost",
                            "direction": "Long" if getattr(trade_obj, "size", 0) > 0 else "Short",
                            "regime": "unknown"
                        }

                        # Add indicator values for entry and exit
                        if data is not None and calculated_indicators:
                            for dt_key, suffix in [(entry_dt, "_entry"), (exit_dt, "_exit")]:
                                if dt_key:
                                    closest_idx = data_index.get_indexer([dt_key], method='nearest')[0]
                                    for ind_name, ind_data in calculated_indicators.items():
                                        ind_value = ind_data['values'].iloc[closest_idx] if closest_idx < len(ind_data['values']) else None
                                        trade_info[f"{ind_data['name']}{suffix}"] = ind_value
        ...
        if not trades and hasattr(strategy_result, "analyzers"):
            try:
                trade_analyzer = getattr(strategy_result.analyzers, "tradeanalyzer", None)
                if trade_analyzer and hasattr(trade_analyzer, "get_analysis"):
                    analysis = trade_analyzer.get_analysis()
                    closed_trades = analysis.get("closed", []) or analysis.get("trades", [])
                    for i, trade in enumerate(closed_trades):
                        try:
                            entry_dt = pd.to_datetime(trade['datein'], unit='s').tz_localize('UTC')
                            exit_dt = pd.to_datetime(trade['dateout'], unit='s').tz_localize('UTC')
                            trade_info = {
                                "entry_time": entry_dt,
                                "exit_time": exit_dt,
                                ...
                                "pnl": trade.get("pnl", 0),
                                "pnl_net": trade.get("pnl", 0),  # TradeAnalyzer may not include commission
                                "commission": 0,
        ...
        if not trades_df.empty:
            logger.info(f"Extracted {len(trades_df)} trades with columns: {trades_df.columns.tolist()}")
        return trades_df
...
def create_trades_table(results, data=None):
    ...
    try:
        strategy = get_first_strategy(results)
        logger.info(f"[create_trades_table] Using strategy: {strategy.__class__.__name__}")
        
        # Extract trades using the robust method
        trades_df = extract_trades(strategy, data)
        
        if trades_df.empty:
            logger.warning("[create_trades_table] No trades extracted using extract_trades")
            return pd.DataFrame(), "No trades executed during the period"
        
        # Convert to IST timezone for display
        trades_df['entry_time'] = pd.to_datetime(trades_df['entry_time']).dt.tz_convert(IST)
        trades_df['exit_time'] = pd.to_datetime(trades_df['exit_time']).dt.tz_convert(IST)
        ...
        for i, trade in trades_df.iterrows():
            ...
            return_pct = (trade['pnl_net'] / (trade['entry_price'] * trade['size'])) * 100 if trade['entry_price'] * trade['size'] != 0 else 0
            ...
            for col in trades_df.columns:
                print(f"Processing column: {trades_df.columns}")
                if col.endswith('_entry') or col.endswith('_exit'):
                    indicator_name = col.rsplit('_', 1)[0]  # Extract base indicator name
                    value = trade[col]
                    # Add both the indicator name and its value to the table
                    if pd.notnull(value):
                        trade_info[f"{indicator_name} ({col.split('_')[-1].capitalize()})"] = f"{value:.2f}" if isinstance(value, (int, float)) else str(value)
                    else:
                        trade_info[f"{indicator_name} ({col.split('_')[-1].capitalize()})"] = "N/A"
...
def safe_display_dataframe(df, **kwargs):
    # Convert all columns to string if they are not scalar
...
def analyze_best_time_ranges(results, data=None):
    ...
    try:
        ...
        trades_df = extract_trades(strategy, data)
        if trades_df.empty:
            logger.warning("[analyze_best_time_ranges] No trade data available (trades_df is empty)")
            return {"error": "No trade data available"}
        ...
        if winning_trades['entry_time'].dt.tz is None:
            winning_trades['entry_time'] = winning_trades['entry_time'].dt.tz_localize('UTC').dt.tz_convert(IST)
        else:
            winning_trades['entry_time'] = winning_trades['entry_time'].dt.tz_convert(IST)
        ...
        hourly_stats = winning_trades.groupby('hour').agg(
            count=('pnl', 'count'),
            total_pnl=('pnl', 'sum')
        ).to_dict('index')
        best_hours = sorted(hourly_stats.items(), key=lambda x: x[1]['count'], reverse=True)[:3]
        # Analyze by day of week
        daily_stats = winning_trades.groupby('day_of_week').agg(
            count=('pnl', 'count'),
            total_pnl=('pnl', 'sum')
        ).to_dict('index')
        best_days = sorted(daily_stats.items(), key=lambda x: x[1]['count'], reverse=True)[:3]
        # Analyze by month
        monthly_stats = winning_trades.groupby('month').agg(
            count=('pnl', 'count'),
            total_pnl=('pnl', 'sum')
        ).to_dict('index')
        best_months = sorted(monthly_stats.items(), key=lambda x: x[1]['count'], reverse=True)[:3]
        return {
            "total_winning_trades": len(winning_trades),
            "best_hours": [{"hour": f"{hour}:00", "trades": stats['count'], "total_pnl": stats['total_pnl']}
                          for hour, stats in best_hours],
            "best_days": [{"day": day, "trades": stats['count'], "total_pnl": stats['total_pnl']}
                         for day, stats in best_days],
            "best_months": [{"month": month, "trades": stats['count'], "total_pnl": stats['total_pnl']}
...
def create_best_times_table(time_analysis):
    ...
    try:
        ...
        if time_analysis.get("hourly_distribution"):
            for hour, stats in time_analysis["hourly_distribution"].items():
                hours_data.append({
                    ...
                    'Avg P&L per Trade': f"{stats['total_pnl']/stats['count']:.2f}" if stats['count'] > 0 else "0.00"
                })
        hours_df = pd.DataFrame(hours_data).sort_values('Winning Trades', ascending=False) if hours_data else pd.DataFrame()
        ...
        days_df = pd.DataFrame(days_data).sort_values('Winning Trades', ascending=False) if days_data else pd.DataFrame()
        ...
        months_df = pd.DataFrame(months_data).sort_values('Winning Trades', ascending=False) if months_data else pd.DataFrame()
...
def create_parameters_table(best_params_info):
    ...
    try:
        ...
        for param, value in best_params_info["best_parameters"].items():
            ...
            params_data.append({
                'Parameter': param,
                'Best Value': f"{value:.4f}" if isinstance(value, float) else str(value),
                'Importance': f"{importance:.3f}" if importance else "N/A"
            })
        
        df = pd.DataFrame(params_data).sort_values('Importance', ascending=False, key=lambda x: pd.to_numeric(x.replace('N/A', '0'), errors='coerce'))
        return df
...
def create_dynamic_indicators_table(data, strategy):
    ...
    try:
        ...
        for i in range(min(5, len(data))):
            ...
            row_data = {
                'Date/Time': date,
                'Close Price': data['Close'].iloc[idx]
            }
            
            # Add all detected indicators
            for indicator_name, indicator_data in calculated_indicators.items():
                if not indicator_data['values'].empty and idx < len(indicator_data['values']):
            ...
            # EMA signals
            ema_indicators = [k for k in calculated_indicators.keys() if 'ema' in k.lower()]
            if len(ema_indicators) >= 2:
                ...
                if fast_ema is not None and slow_ema is not None:
                    signals.append('EMA: Bullish' if fast_ema > slow_ema else 'EMA: Bearish')
            
            # RSI signals
            rsi_indicators = [k for k in calculated_indicators.keys() if 'rsi' in k.lower()]
            ...
            # MACD signals
            macd_line_indicators = [k for k in calculated_indicators.keys() if 'macd' in k.lower() and 'line' in k.lower()]
            macd_signal_indicators = [k for k in calculated_indicators.keys() if 'macd' in k.lower() and 'signal' in k.lower()]
            
            if macd_line_indicators and macd_signal_indicators:
                macd_line = calculated_indicators[macd_line_indicators[0]]['values'].iloc[idx]
                macd_signal = calculated_indicators[macd_signal_indicators[0]]['values'].iloc[idx]
                signals.append('MACD: Bullish' if macd_line > macd_signal else 'MACD: Bearish')
...
def create_strategy_comparison_table(basic_report, opt_report):
    ...
    try:
        ...
        metrics = [
            ("Total Return (%)", basic_summary.get("total_return_pct", 0), opt_summary.get("total_return_pct", 0)),
            ("Sharpe Ratio", basic_summary.get("sharpe_ratio", 0), opt_summary.get("sharpe_ratio", 0)),
            ("Max Drawdown (%)", basic_summary.get("max_drawdown_pct", 0), opt_summary.get("max_drawdown_pct", 0)),
            ("Total Trades", basic_trades.get("total_trades", 0), opt_trades.get("total_trades", 0)),
            ("Win Rate (%)", basic_trades.get("win_rate_percent", 0), opt_trades.get("win_rate_percent", 0)),
            ("Profit Factor", basic_trades.get("profit_factor", 0), opt_trades.get("profit_factor", 0)),
            ("Final Value ()", basic_summary.get("final_value", 0), opt_summary.get("final_value", 0))
        ]
        
        for metric, basic_val, opt_val in metrics:
            improvement = ""
            if isinstance(basic_val, (int, float)) and isinstance(opt_val, (int, float)) and basic_val != 0:
            ...
            comparison_data.append({
                "Metric": metric,
                "Basic Strategy": f"{basic_val:.2f}" if isinstance(basic_val, float) else str(basic_val),
                "Optimized Strategy": f"{opt_val:.2f}" if isinstance(opt_val, float) else str(opt_val),
...
def plot_results(cerebro, data, results):
    """Enhanced plot results function that creates candlestick charts."""
    try:
...
def plot_time_analysis(time_analysis):
    ...
    try:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=("Winning Trades by Hour", "Winning Trades by Day", 
                          "P&L by Hour", "P&L by Day"),
        ...
        if time_analysis.get("hourly_distribution"):
            ...
            counts = [stats['count'] for stats in time_analysis["hourly_distribution"].values()]
            fig.add_trace(
                go.Bar(x=[f"{h}:00" for h in hours], y=counts, name="Trades by Hour", 
        ...
        if time_analysis.get("daily_distribution"):
            ...
            counts = [stats['count'] for stats in time_analysis["daily_distribution"].values()]
            fig.add_trace(
                go.Bar(x=days, y=counts, name="Trades by Day", 
        ...
        if time_analysis.get("hourly_distribution"):
            hours = list(time_analysis["hourly_distribution"].keys())
            pnls = [stats['total_pnl'] for stats in time_analysis["hourly_distribution"].values()]
            fig.add_trace(
                go.Bar(x=[f"{h}:00" for h in hours], y=pnls, name="P&L by Hour", 
                       marker_color='orange'),
                row=2, col=1
            )
        ...
        if time_analysis.get("daily_distribution"):
            days = list(time_analysis["daily_distribution"].keys())
            pnls = [stats['total_pnl'] for stats in time_analysis["daily_distribution"].values()]
            fig.add_trace(
                go.Bar(x=days, y=pnls, name="P&L by Day", 
                       marker_color='salmon'),
                row=2, col=2
            )
...
def complete_backtest(user_ticker=None, user_start_date=None, user_end_date=None, user_strategy=None, user_timeframe=None, user_analyzers=None):
    ...
    try:
        ...
        start_date = user_start_date if user_start_date else (datetime.today().date() - timedelta(days=60)).strftime("%Y-%m-%d")
        end_date = user_end_date if user_end_date else (datetime.today().date() - timedelta(days=2)).strftime("%Y-%m-%d")
        timeframe = user_timeframe if user_timeframe else "5m"
        strategy_name = user_strategy if user_strategy else list(STRATEGY_REGISTRY.keys())[0]
        selected_analyzers = user_analyzers if user_analyzers else ["SharpeRatio", "DrawDown", "Returns", "TradeAnalyzer", "TimeReturn", "SortinoRatio"]
        ...
        analyzer_config = [(available_analyzers[name], {"_name": name.lower()}) for name in selected_analyzers]
        ...
        if data[required_columns].isnull().any().any():
            ...
            return
        if not data[required_columns].apply(lambda x: x.apply(lambda y: isinstance(y, (int, float)))).all().all():
        ...
        # 1. Run Backtest
        status_text.text("Running demo backtest...")
        ...
        backtest_report = analyzer.generate_full_report()
        st.write("### Backtest Results")
        # Show each section of the report as a table
        if isinstance(backtest_report, dict):
            ...
            trade_analysis = backtest_report.get("trade_analysis", {})
            if trade_analysis and not trade_analysis.get("error"):
                st.write("#### Trade Analysis")
                st.dataframe(pd.DataFrame([trade_analysis]).T, use_container_width=True)
            risk_metrics = backtest_report.get("risk_metrics", {})
            if risk_metrics and not risk_metrics.get("error"):
                st.write("#### Risk Metrics")
                st.dataframe(pd.DataFrame([risk_metrics]).T, use_container_width=True)
            resampled_returns = backtest_report.get("resampled_returns", {})
            if resampled_returns and not resampled_returns.get("error"):
                st.write("#### Resampled Returns (Summary)")
                summary_keys = [k for k in resampled_returns.keys() if k != "resampled_returns_pct"]
                # Convert all values to string to avoid Arrow type errors
                summary_dict = {k: str(resampled_returns[k]) for k in summary_keys}
                st.dataframe(pd.DataFrame([summary_dict]).T, use_container_width=True)
                if "resampled_returns_pct" in resampled_returns:
                    st.write("#### Resampled Returns (%) by Period")
                    rr_df = pd.DataFrame(list(resampled_returns["resampled_returns_pct"].items()), columns=["Period", "Return %"])
                    st.dataframe(rr_df, use_container_width=True)
            drawdown_analysis = backtest_report.get("drawdown_analysis", {})
            if drawdown_analysis and not drawdown_analysis.get("error"):
                st.write("#### Drawdown Analysis")
                st.dataframe(pd.DataFrame([drawdown_analysis]).T, use_container_width=True)
        st.write("#### Full JSON Report")
        st.json(backtest_report)
        
        # Export backtest report
        if st.button("Export Demo Backtest Report"):
            report_json = json.dumps(backtest_report, indent=2, default=str)
            st.download_button(
                label="Download Demo Backtest Report as JSON",
                data=report_json,
                file_name=f"{ticker}_demo_backtest_report.json",
                mime="application/json"
            )
        
        # Enhanced Demo Backtest Chart
        st.write("### 📈 Backtest - Enhanced Chart")
        plotly_fig = create_candlestick_chart_with_trades(data, results, "Demo Backtest Results")
        if plotly_fig:
            st.plotly_chart(plotly_fig, use_container_width=True)
            if st.button("Export Backtest Chart"):
                buf = BytesIO()
                plotly_fig.write_image(buf, format="png")
                st.download_button(
                    label="Download Backtest Chart as PNG",
                    data=buf.getvalue(),
                    file_name=f"{ticker}_backtest_chart.png",
                    mime="image/png"
                )
        
        st.write("### 📊 Backtest - Trades Table (First 10)")
        trades_df, trades_error = create_trades_table(results)
        if not trades_error and not trades_df.empty:
            st.dataframe(trades_df.head(10), use_container_width=True)  # Show first 10 trades
            st.write("### 📋 Detailed Trades Table (All Trades)")
            st.dataframe(trades_df, use_container_width=True)
        else:
            st.info("No trades executed in demo backtest.")

        # Best Trading Times Tables
        ...
        if hours_df is not None and not hours_df.empty:
            st.write("#### Best Hours for Winning Trades")
            st.dataframe(hours_df, use_container_width=True)
        if days_df is not None and not days_df.empty:
            st.write("#### Best Days for Winning Trades")
            st.dataframe(days_df, use_container_width=True)
        if months_df is not None and not months_df.empty:
            st.write("#### Best Months for Winning Trades")
            st.dataframe(months_df, use_container_width=True)
        
        progress_bar.progress(50)
        
        # 2. Run Optimization
        status_text.text("Running demo optimization...")
        opt_results = optimize_strategy(
            strategy_class=get_strategy(strategy_name),
            ticker=ticker,
            start_date=start_date,
            end_date=end_date,
            n_trials=10,  # Limited trials for demo
            interval=timeframe,
            analyzers=analyzer_config
        )
        opt_analyzer = PerformanceAnalyzer(opt_results["results"][0])
        opt_report = opt_analyzer.generate_full_report()
        st.write("### Optimization Results")
        st.json(opt_report)
        
        # Enhanced Demo Optimization Chart
        st.write("### 📈 Optimization - Enhanced Chart")
        demo_opt_fig = create_candlestick_chart_with_trades(data, opt_results["results"], "Demo Optimization Results")
        if demo_opt_fig:
            st.plotly_chart(demo_opt_fig, use_container_width=True)
        
        st.write("###  Optimization - Best Parameters")
        demo_best_params = display_best_parameters(opt_results)
        if "error" not in demo_best_params:
            col1, col2 = st.columns(2)
            
            with col1:
                st.write("**📊 Optimization Summary**")
                st.metric("Best Objective Value", f"{demo_best_params['best_objective_value']:.4f}" if demo_best_params['best_objective_value'] else "N/A")
                st.metric("Success Rate", f"{demo_best_params['success_rate']:.1f}%")
            
            with col2:
                st.write("**⚙️ Parameter Details**")
                demo_params_df = create_parameters_table(demo_best_params)
                if not demo_params_df.empty:
                    st.dataframe(demo_params_df, use_container_width=True)
        
        st.write("### 📊 Strategy Comparison")
        demo_comparison_df = create_strategy_comparison_table(backtest_report, opt_report)
        if not demo_comparison_df.empty:
            st.dataframe(demo_comparison_df, use_container_width=True)
        
        st.write("### 📊 Optimization - Trades Table (First 10)")
        opt_trades_df, opt_trades_error = create_trades_table(opt_results["results"])
        if not opt_trades_error and not opt_trades_df.empty:
            st.dataframe(opt_trades_df.head(10), use_container_width=True)
            st.write("### 📋 Detailed Trades Table (All Trades)")
            st.dataframe(opt_trades_df, use_container_width=True)
        else:
            st.info("No trades executed in demo optimization.")

        st.write("### ⏰ Best Trading Times Analysis (Optimization)")
        opt_time_analysis = analyze_best_time_ranges(opt_results["results"])
        opt_hours_df, opt_days_df, opt_months_df = create_best_times_table(opt_time_analysis)
        if opt_hours_df is not None and not opt_hours_df.empty:
            st.write("#### Best Hours for Winning Trades")
            st.dataframe(opt_hours_df, use_container_width=True)
        if opt_days_df is not None and not opt_days_df.empty:
            st.write("#### Best Days for Winning Trades")
            st.dataframe(opt_days_df, use_container_width=True)
        if opt_months_df is not None and not opt_months_df.empty:
            st.write("#### Best Months for Winning Trades")
            st.dataframe(opt_months_df, use_container_width=True)
        
        # Export optimization report
        if st.button("Export Demo Optimization Report"):
            report_json = json.dumps(opt_report, indent=2, default=str)
            st.download_button(
                label="Download Demo Optimization Report as JSON",
                data=report_json,
                file_name=f"{ticker}_demo_optimization_report.json",
                mime="application/json"
            )
        
        st.write("### 🗺️ Optimization - Parameter Landscape")
        demo_contour_fig = plot_contour(opt_results["study"])
        if demo_contour_fig:
            st.plotly_chart(demo_contour_fig, use_container_width=True)
            if st.button("Export Demo Contour Plot"):
                buf = BytesIO()
                demo_contour_fig.write_image(buf, format="png")
                st.download_button(
                    label="Download Demo Contour Plot as PNG",
                    data=buf.getvalue(),
                    file_name=f"{ticker}_demo_contour_plot.png",
                    mime="image/png"
                )
        ...
        status_text.text("Running demo walk-forward analysis...")
        ...
        if wf_results["windows"]:
            wf_analyzer = PerformanceAnalyzer(wf_results["windows"][-1]["out_sample_performance"])
            wf_report = wf_analyzer.generate_full_report()
            st.write("### Walk-Forward Results")
            st.json(wf_report)

            # Walk-Forward Trades Table
            st.write("### 📊 Walk-Forward - Trades Table (First 10)")
            wf_trades_df, wf_trades_error = create_trades_table(wf_results["windows"][-1]["out_sample_performance"])
            if not wf_trades_error and not wf_trades_df.empty:
                st.dataframe(wf_trades_df.head(10), use_container_width=True)
                st.write("### 📋 Detailed Trades Table (All Trades)")
                st.dataframe(wf_trades_df, use_container_width=True)
            else:
                st.info("No trades executed in walk-forward analysis.")

            st.write("### ⏰ Best Trading Times Analysis (Walk-Forward)")
            wf_time_analysis = analyze_best_time_ranges(wf_results["windows"][-1]["out_sample_performance"])
            wf_hours_df, wf_days_df, wf_months_df = create_best_times_table(wf_time_analysis)
            if wf_hours_df is not None and not wf_hours_df.empty:
                st.write("#### Best Hours for Winning Trades")
                st.dataframe(wf_hours_df, use_container_width=True)
            if wf_days_df is not None and not wf_days_df.empty:
                st.write("#### Best Days for Winning Trades")
                st.dataframe(wf_days_df, use_container_width=True)
            if wf_months_df is not None and not wf_months_df.empty:
                st.write("#### Best Months for Winning Trades")
                st.dataframe(wf_months_df, use_container_width=True)
            
            # Export walk-forward report
            if st.button("Export Demo Walk-Forward Report"):
                report_json = json.dumps(wf_report, indent=2, default=str)
                st.download_button(
                    label="Download Demo Walk-Forward Report as JSON",
                    data=report_json,
                    file_name=f"{ticker}_demo_walkforward_report.json",
                    mime="application/json"
                )
            
            # Plot walk-forward equity curve
            if "resampled_returns" in wf_results["windows"][-1]["out_sample_performance"]:
                equity_data = wf_results["windows"][-1]["out_sample_performance"]["resampled_returns"]
                
                # Handle different data formats
                if isinstance(equity_data, dict):
                    try:
                        # Attempt to parse keys as datetimes
                        dates = pd.to_datetime(list(equity_data.keys()), errors='coerce')
                        values = list(equity_data.values())
                        # Filter out invalid dates
                        valid_mask = ~dates.isnull()
                        dates = dates[valid_mask]
                        values = [v for i, v in enumerate(values) if valid_mask[i]]
                    except Exception:
                ...
                if len(dates) > 0 and values:
                    # Convert to IST timezone if possible
                    if isinstance(dates, pd.DatetimeIndex):
                        try:
                            if dates.tz is None:
                                dates = dates.tz_localize('UTC').tz_convert(IST)
                            else:
                                dates = dates.tz_convert(IST)
                        except:
                            pass
                    
                    wf_plotly_fig = go.Figure()
                    wf_plotly_fig.add_trace(
                        go.Scatter(x=dates, y=values, mode='lines', name='Equity Curve')
                    )
                    wf_plotly_fig.update_layout(
                        title="Demo Walk-Forward Equity Curve",
                        xaxis_title="Date",
                        yaxis_title="Equity",
                        height=600
                    )
                    st.plotly_chart(wf_plotly_fig)
                    
                    if st.button("Export Demo Walk-Forward Equity Curve Plot"):
                        buf = BytesIO()
                        wf_plotly_fig.write_image(buf, format="png")
                        st.download_button(
                            label="Download Demo Walk-Forward Equity Curve as PNG",
                            data=buf.getvalue(),
                            file_name=f"{ticker}_demo_walkforward_plot.png",
                            mime="image/png"
                        )
...
def main():
    ...
    **Advanced Trading Strategy Analysis Platform**
    ...
    with st.sidebar.expander("Ticker Management", expanded=False):
        ...
        st.write(", ".join(current_tickers[:10]) + ("..." if len(current_tickers) > 10 else ""))
        
        # Add new ticker
        new_ticker = st.text_input("Add New Ticker", help="Enter ticker symbol to add to the list")
        ...
        if uploaded_file is not None:
            try:
                ...
                imported_tickers = [line.strip().upper() for line in content.split('\n') if line.strip()]
                ...
                if valid_tickers:
                    if save_tickers_to_file(valid_tickers):
                        ...
                        if invalid_tickers:
                            st.warning(f"Skipped {len(invalid_tickers)} invalid tickers: {', '.join(invalid_tickers[:5])}")
                        st.experimental_rerun()
                    else:
    ...
    start_date = st.sidebar.date_input("Start Date", value=start_date_default, max_value=end_date_default - timedelta(days=1))
    end_date = st.sidebar.date_input("End Date", value=end_date_default, min_value=start_date + timedelta(days=1), max_value=datetime.today().date())
    ...
    # Analysis type
    analysis_type = st.sidebar.selectbox("Analysis Type", ["Backtest", "Optimization", "Walk-Forward", "Complete Backtest"])
    ...
    selected_analyzers = st.sidebar.multiselect(
        ...
        default=["SharpeRatio", "DrawDown", "Returns", "TradeAnalyzer", "TimeReturn", "SortinoRatio"]
    )
    ...
    if st.sidebar.button("Run Analysis"):
        try:
            ...
            if data.empty:
                ...
                status_text.text("No data available for the selected ticker and date range.")
                return
            ...
            if analysis_type == "Backtest":
                ...
                plotly_fig = plot_results(cerebro, data, results)
                print(f"Plotly figure created: {plotly_fig}, {type(plotly_fig)}")
                ...
                if not indicators_df.empty:
                    ...
                    if detected_indicators:
                        ...
                        for name, info in detected_indicators.items():
                            ...
                            params = info.get('params', {})
                            user_params = {k: v for k, v in params.items() if not (k.startswith('_') or callable(v) or isinstance(v, type) or 'method' in str(type(v)).lower())}
                            if user_params:
                                params_str = ", ".join([f"{k} = {v}" for k, v in user_params.items()])
                            else:
                                params_str = "No parameters"
                            indicator_summary.append(f"- **{info['type']}** (`{name}`): {params_str}")
                ...
                if "error" not in best_trades_analysis:
                    ...
                    with col2:
                        win_rate = (best_trades_analysis['winning_trades']/best_trades_analysis['total_trades']*100) if best_trades_analysis["total_trades"] > 0 else 0
                    ...
                    with col4:
                        st.metric("Avg Winning Trade", f"{best_trades_analysis['avg_winning_trade']:.2f}")
                        st.metric("Avg Losing Trade", f"{best_trades_analysis['avg_losing_trade']:.2f}")
                ...
                plotly_fig = create_candlestick_chart_with_trades(data, [strategy_result], "Optimized Strategy Results")
                if plotly_fig:
                    st.plotly_chart(plotly_fig, use_container_width=True)
                    ...
                    if opt_detected_indicators:
                        ...
                        for name, info in opt_detected_indicators.items():
                            ...
                            user_params = {
                                ...
                                if not (k.startswith('_') or callable(v) or isinstance(v, type) or 'method' in str(type(v)).lower())
                            }
                            if user_params:
                                params_str = ", ".join([f"{k}={v}" for k, v in user_params.items()])
                            else:
                                params_str = "No parameters"
                            opt_indicator_summary.append(f"- **{info['type']}** (`{name}`): {params_str}")
                ...
                if "error" not in best_params_info:
                    ...
                    with col1:
                        st.write("**📊 Optimization Summary**")
                        st.metric("Best Objective Value", f"{best_params_info['best_objective_value']:.4f}" if best_params_info['best_objective_value'] else "N/A")
                        st.metric("Success Rate", f"{best_params_info['success_rate']:.1f}%")
                        st.metric("Completed Trials", f"{best_params_info['completed_trials']}/{best_params_info['total_trials']}")
                    ...
                    st.warning("Could not display best parameters: " + best_params_info.get("error", "Unknown error"))
                ...
                if "error" not in time_analysis:
                    ...
                    for hour_info in time_analysis["best_hours"]:
                        st.write(f"  - {hour_info['hour']}: {hour_info['trades']} trades ({hour_info['total_pnl']:.2f})")
                    st.write("- Best Days:")
                    for day_info in time_analysis["best_days"]:
                        st.write(f"  - {day_info['day']}: {day_info['trades']} trades ({day_info['total_pnl']:.2f})")
                    st.write("- Best Months:")
                    for month_info in time_analysis["best_months"]:
                        st.write(f"  - {month_info['month']}: {month_info['trades']} trades ({month_info['total_pnl']:.2f})")
                else:
                    st.warning("Could not analyze best time to trade: " + time_analysis.get("error", "Unknown error"))

                # Best holding period
                best_trades_analysis = analyze_best_trades([strategy_result])
                if "error" not in best_trades_analysis and best_trades_analysis["best_trades"]:
                    best_trade = best_trades_analysis["best_trades"][0]
                    avg_holding = (
                        sum(trade["duration_hours"] for trade in best_trades_analysis["best_trades"]) /
                        len(best_trades_analysis["best_trades"])
                    )
                    st.write("**Best Holding Period:**")
                    st.write(f"- Best Trade Holding: {best_trade['duration_hours']:.2f} hours")
                    st.write(f"- Average Holding (Top 5): {avg_holding:.2f} hours")
                else:
                    st.warning("Could not analyze holding period: " + best_trades_analysis.get("error", "Unknown error"))
                ...
                if "error" not in time_analysis:
                    ...
                    st.warning("Could not analyze time ranges: " + time_analysis.get("error", "Unknown error"))
                ...
                if results["windows"]:
                    analyzer = PerformanceAnalyzer(results["windows"][-1]["out_sample_performance"])
                    report = analyzer.generate_full_report()
                    st.write("### Walk-Forward Results")
                    st.json(report)
                    ...
                    with col1:
                        ...
                        if results["windows"]:
                            ...
                            avg_performance = sum(window.get("out_sample_performance", {}).get("total_return_pct", 0) 
                                                for window in results["windows"]) / len(results["windows"])
                            st.metric("Average Return per Window", f"{avg_performance:.2f}%")
                    
                    with col2:
                        st.write("**Best Window Performance:**")
                        if results["windows"]:
                            best_window = max(results["windows"], 
                                            key=lambda x: x.get("out_sample_performance", {}).get("total_return_pct", 0))
                            best_return = best_window.get("out_sample_performance", {}).get("total_return_pct", 0)
                    ...
                    # Plot equity curve
                    if "resampled_returns" in results["windows"][-1]["out_sample_performance"]:
                        equity_data = results["windows"][-1]["out_sample_performance"]["resampled_returns"]
                        ...
                        if isinstance(equity_data, dict):
                            ...
                            for k, v in equity_data.items():
                                if isinstance(v, dict):
                                    ...
                                    break
                            if ts_key:
                                logger.info(f"Equity data contains time series key: {ts_key}")
                                equity_data = equity_data[ts_key]
                        # Now process as before if it's a dict of date:value
                        if isinstance(equity_data, dict):
                            filtered = [(k, v) for k, v in equity_data.items() if not isinstance(k, str) or not k.lower().startswith("return")]
                            if filtered:
                                date_keys, values = zip(*filtered)
                                equity_dates = pd.to_datetime(list(date_keys), errors='coerce')
                                logger.info(f"Equity values (raw): {values}")
                                equity_values = [v["returns"] if isinstance(v, dict) and "returns" in v else v for v in values]
                                # Remove any NaT dates and corresponding values
                                mask = ~pd.isnull(equity_dates)
                                equity_dates = equity_dates[mask]
                                equity_values = [v for i, v in enumerate(equity_values) if mask[i]]
                            else:
                                equity_dates = []
                                equity_values = []
                        elif hasattr(equity_data, 'index') and hasattr(equity_data, 'values'):
                            equity_dates = pd.to_datetime(equity_data.index).tz_convert(IST)
                        ...
                        plotly_fig.add_trace(
                            go.Scatter(x=equity_dates, y=equity_values, mode='lines', name='Equity Curve')
                        )
                        plotly_fig.update_layout(
