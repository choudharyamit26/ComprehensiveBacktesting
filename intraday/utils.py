...
def get_index_signal_dhan_api(
    ...
    try:
        ...
        yesterday_response = requests.post(
            url, headers=headers, json=yesterday_payload, timeout=30
        )
        yesterday_data = yesterday_response.json()

        if not yesterday_data.get("close"):
            logger.error(f"No data available for {index_name} on {yesterday_str}")
            return {
                "signal": "ERROR",
                "message": "No previous day data available",
                "change": None,
            }

        # Get yesterday's closing price
        yesterday_close = float(yesterday_data["close"][-1])
        logger.info(
            f"Yesterday's close for {index_name} on {yesterday_str}: {yesterday_close} and today's most recent close: {most_recent_close}"
        )
        # Calculate change points from yesterday's close to today's most recent price
        change_points = round(most_recent_close - yesterday_close, 2)
        change_pct = round((change_points / yesterday_close) * 100, 2)

        # Calculate threshold in points (percentage-based)
        threshold_points = yesterday_close * threshold / 100

        # Generate signal based on percentage change
        if change_points > 70:
            signal = "BUY"
        elif change_points < -70:
            signal = "SELL"
        else:
            signal = "HOLD"
