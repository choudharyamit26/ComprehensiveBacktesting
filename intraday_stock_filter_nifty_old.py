...
from comprehensive_backtesting.backtesting import run_basic_backtest, run_parameter_optimization
...
def run_complete_backtests(selected_stocks, strategies):
    from app import create_consolidated_metrics, extract_report_metrics, generate_strategy_report

    end_date = datetime.today().date() - timedelta(days=1)
    start_date = end_date - timedelta(days=365)
    interval="5m"
    n_trials = 20
    selected_stocks = [stock["Stock"] for stock in selected_stocks]
    for ticker in selected_stocks:
        ...
        for strategy in strategies:
            ...
            strategy_report = generate_strategy_report(
                results, strategy, ticker, interval
            )
            if strategy_report:
                bt_strategy_reports.append(strategy_report)
            ...
            if strategy_report:
                opt_strategy_reports.append(strategy_report)
        ...
        for i, window in enumerate(wf.results):
            if window.get("valid") and "out_sample_performance" in window:
                ...
                if report:
                    wf_strategy_reports.append(report)
    all_metrics = []
    strategy_metrics = {}
    # Store backtest metrics
    for report in bt_strategy_reports:
        strategy_name = report["Strategy"]
        key = (ticker, strategy_name)
        strategy_metrics[key] = {"backtest": extract_report_metrics(report)}

    # Store optimization metrics
    for report in opt_strategy_reports:
        strategy_name = report["Strategy"]
        key = (ticker, strategy_name)
        params_dict = report.get("Params", {})  # Extract optimized parameters
    ...
    for report in wf_strategy_reports:
        strategy_name = report["Strategy"]
        key = (ticker, strategy_name)
        params_dict = report.get("Params", {})  # Extract optimized parameters
        if key in strategy_metrics:
    ...
    if all_metrics:
        ...
        # Extract top 3 strategies per stock with optimized parameters
        for ticker in consolidated_df["Ticker"].unique():
            ...
            # Get optimized parameters from strategy_metrics
            top_strategies_list = []
            for _, row in top_strategies.iterrows():
                strategy_name = row["Strategy"]
                key = (ticker, strategy_name)
                best_params = {}
                # Prefer walk-forward parameters if available, else optimization
                if key in strategy_metrics and "walkforward" in strategy_metrics[key]:
                    best_params = strategy_metrics[key]["walkforward"].get("best_params", {})
                elif key in strategy_metrics and "optimization" in strategy_metrics[key]:
                    best_params = strategy_metrics[key]["optimization"].get("best_params", {})
    ...
    top_strategies_df["Best_Parameters"] = top_strategies_df["Best_Parameters"].apply(
...
if __name__ == "__main__":
    ...
    selected_stocks = select_stocks_for_intraday(csv_file="ind_nifty50list.csv")
