...
class HeadShouldersConfirmation(bt.Strategy):
    """
    Head & Shoulders + Confirmation Strategy
    Strategy Type: INTRADAY REVERSAL
    =================================
    Detects Head & Shoulders pattern with neckline break, confirmed by RSI and volume surge.

    Strategy Logic:
    ==============
    Long Entry: Neckline break upward + RSI > 30 + Volume > 1.5 * SMA(Volume)
    Short Entry: Neckline break downward + RSI < 70 + Volume > 1.5 * SMA(Volume)
    Exit: Pattern target (measured move) or neckline retest (failure)

    Parameters:
    ==========
    - rsi_period (int): RSI period (default: 14)
    - volume_sma_period (int): Volume SMA period for surge detection (default: 20)
    - lookback (int): Bars to look back for pattern detection (default: 20)
    ...
    def detect_head_shoulders(self):
        """Detect Head & Shoulders pattern within lookback period."""
        ...
        # Simplified detection: look for three peaks (left shoulder, head, right shoulder)
        mid = lookback // 2
        head = max(highs)
        head_idx = highs.index(head)

        if head_idx <= 2 or head_idx >= lookback - 3:
            return None, None

        left_shoulder = max(highs[:head_idx][-3:])
        right_shoulder = max(highs[head_idx + 1 :][:3])
        if left_shoulder >= head or right_shoulder >= head:
            return None, None

        # Find neckline (average of lows between shoulders and head)
        neckline_lows = lows[head_idx - 2 : head_idx + 3]
        ...
        target = (
            neckline + (head - neckline) * self.params.target_multiplier
            if direction == "bullish"
            else neckline - (head - neckline) * self.params.target_multiplier
        )
    ...
    def next(self):
        ...
        direction, target = self.detect_head_shoulders()
        volume_surge = self.datas[0].volume[0] > self.volume_sma[0] * 1.5

        if not self.position:
            if direction == "bullish" and self.rsi[0] > 30 and volume_surge:
                self.order = self.buy()
                self.order_type = "enter_long"
                self.target_price = target
                trade_logger.info(
                    f"BUY SIGNAL (Head & Shoulders) | Bar: {len(self)} | Time: {bar_time_ist} | "
                    f"Price: {self.datas[0].close[0]:.2f} | RSI: {self.rsi[0]:.2f} | Volume Surge: {volume_surge}"
                )
            elif direction == "bearish" and self.rsi[0] < 70 and volume_surge:
                self.order = self.sell()
                self.order_type = "enter_short"
                self.target_price = target
                trade_logger.info(
                    f"SELL SIGNAL (Head & Shoulders) | Bar: {len(self)} | Time: {bar_time_ist} | "
                    f"Price: {self.datas[0].close[0]:.2f} | RSI: {self.rsi[0]:.2f} | Volume Surge: {volume_surge}"
                )
        elif self.position.size > 0:
            if self.datas[0].close[0] >= self.target_price or self.rsi[0] < 30:
                self.order = self.sell()
                self.order_type = "exit_long"
                trade_logger.info(
                    f"SELL SIGNAL (Exit Long) | Bar: {len(self)} | Time: {bar_time_ist} | "
                    f"Price: {self.datas[0].close[0]:.2f} | Reason: {'Target reached' if self.datas[0].close[0] >= self.target_price else 'RSI < 30'}"
                )
        elif self.position.size < 0:
            if self.datas[0].close[0] <= self.target_price or self.rsi[0] > 70:
                self.order = self.buy()
                self.order_type = "exit_short"
                trade_logger.info(
                    f"BUY SIGNAL (Exit Short) | Bar: {len(self)} | Time: {bar_time_ist} | "
                    f"Price: {self.datas[0].close[0]:.2f} | Reason: {'Target reached' if self.datas[0].close[0] <= self.target_price else 'RSI > 70'}"
                )
